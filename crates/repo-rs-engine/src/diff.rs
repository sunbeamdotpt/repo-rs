...
/// Trait for diff logic.
#[async_trait::async_trait]
pub trait Diff {
    /// Show diffs for the given projects.
    async fn diff(
        &self,
        ctx: &Context,
...
/// Default diff implementation.
pub struct DefaultDiff;

#[async_trait::async_trait]
impl Diff for DefaultDiff {
    async fn diff(
        &self,
        ctx: &Context,
        projects: Vec<Utf8PathBuf>,
        ...
        for path in projects {
            ...
            if dirty {
                lines.push(format!("{}: (dirty)", project.name));
            }
        }
        Ok(lines.join("\n"))
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_diff_dirty_project() {
        ...
        let engine = DefaultDiff;
        let text = engine
            .diff(&ctx, vec![Utf8PathBuf::from("foo")], DiffOptions::default())
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_diff_clean_project() {
        let tmp = tempfile::tempdir().unwrap();
        let mut ctx = make_context(&tmp);
        let mut mock = MockGitBackend::new();
        mock.expect_open()
    ...
    #[tokio::test]
    async fn test_diff_missing_project() {
        let tmp = tempfile::tempdir().unwrap();
        let ctx = make_context(&tmp);
        let engine = DefaultDiff;
        let err = engine
            .diff(&ctx, vec![Utf8PathBuf::from("missing")], DiffOptions::default())
            .await
            .unwrap_err();
